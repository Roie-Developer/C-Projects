...
int countIllegalEntries() {
	...
	for (i = 0; i < g_entryLabelsNum; i++) {
		...
		if (label) {
			if (label->isExtern) {
				printError(g_entryLines[i]->lineNum,
						"The parameter for .entry can't be an external label.");
				ret++;
			}
		} else {
			printError(g_entryLines[i]->lineNum, "No such label as \"%s\".",
					g_entryLines[i]->lineStr);
			ret++;
		}
	}
...
/* Returns a memory word which represents the operand (assuming it is a valid operand). */
memoryWord getOpMemoryWord(operandInfo op, bool isDest, int lineNum) {
	memoryWord memory = { 0 };
	/* Check if it is a register or not */
	if (op.type == REGISTER) {
		memory.era = (eraType) ABSOLUTE; /* Registers are absolute */

		/* Check if it is the dest or src */
		if (isDest) {
		...
		if(!checkMatAndGetValue(&op, lineNum)){
			printError(lineNum, "Error");
		}
	}else {
		labelInfo *label = getLabel(op.str);
		/* Set era */
		if (op.type == LABEL && label && label->isExtern) {
			...
			memory.era =
					(op.type == NUMBER) ?
							(eraType) ABSOLUTE : (eraType) RELOCATABLE;
		}
...
void addWordToMemory(int *memoryArr, int *memoryCounter, memoryWord memory) {
	/* Check if memoryArr is not full yet */
...
bool addLineToMemory(int *memoryArr, int *memoryCounter, lineInfo *line) {
	...
	/* Don't do anything if the line is error or if it is not a command line */
	if (!line->isError && line->cmd != NULL) {
		/* Update the label operands value */
		if (!updateLableOpAddress(&line->op1, line->lineNum)
		...
		if (line->op1.type == REGISTER && line->op2.type == REGISTER) {
			...
			if (line->op1.type != INVALID) {
				...
				addWordToMemory(memoryArr, memoryCounter,
						getOpMemoryWord(line->op1, FALSE, line->lineNum));
				/* ^^ The FALSE param means it is not the 2nd op */
			}
			...
			if (line->op2.type != INVALID) {
				...
				addWordToMemory(memoryArr, memoryCounter,
						getOpMemoryWord(line->op2, TRUE, line->lineNum));
				/* ^^ The TRUE param means it is the 2nd op */
			}
		}
	}
